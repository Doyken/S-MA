...
def test_process(opt, critertion, cl_model, test_loader,epoch,last_F1=None, log_summary_writer: SummaryWriter=None, gamma_1=1.0):
    y_true = []
    y_pre = []
    total_labels = 0
    test_loss = 0
    ...
    with torch.no_grad():
        ...
        if test_loader_tqdm.total is None:
            raise ValueError("test_loader_tqdm.total is None. Check the initialization of test_loader_tqdm.")
        ...
        for index, data in enumerate(test_loader_tqdm):
            ...
            train_test = False
            t_log_prob, i_log_prob, all_log_prob, all_prob, kl_t_log_prob, kl_a_log_prob, kl_all_prob= cl_model(
                orgin_param, index, epoch,train_test)
